...
plot_roc = function(sens_spec_df , 
                    title)
  {
  ...
  cf_mat_normalized = 
    ...
    mutate(Truth = c('ckd',
           ...
           Prediction = c('ckd',
                          ...
                          'ckd',
                          'notckd'),
           Values = c(round(value[1] / (value[1] + value[2]), 
                            2),
                      round(value[2] / (value[1] + value[2]), 
                            2),
                      round(value[3] / (value[3] + value[4]), 
                            2),
                      round(value[4] / (value[3] + value[4]), 
                            2))) %>% 
    select(!c(name,value))
